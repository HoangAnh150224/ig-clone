...
import { 
  Box, Flex, VStack, Text, HStack, Button, Input, Textarea, 
  NativeSelect, Separator, Icon, Center, IconButton 
...
const SettingsItem = ({ icon: IconComponent, label, isActive, onClick }) => (
  <HStack
    px={8} py={4} cursor="pointer" bg={isActive ? "gray.50" : "transparent"}
    _hover={{ bg: "gray.50" }} onClick={onClick} borderLeft={isActive ? "2px solid black" : "none"}
    transition="all 0.2s"
  >
    <Box as={IconComponent} size={24} />
    <Text fontSize="16px" fontWeight={isActive ? "bold" : "400"}>{label}</Text>
  </HStack>
);
...
const Settings = () => {
  ...
  // Form State
  const [formData, setFormData] = useState({ website: '', bio: '', gender: '' });
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      setIsPrivate(user.privacy?.isPrivateAccount || false);
      setBlockedUsers(user.social?.blockedUsers || []);
    }

    setGenderOptions(userService.getGenderOptions());
    fetchFavorites();
  }, []);
  ...
  const fetchFavorites = async () => {
    try {
      ...
      // Mock mapping logic for display
      const mockData = [
        { id: 'u-002', username: 'leomessi', fullName: 'Leo Messi', avatar: 'https://i.pravatar.cc/150?u=messi' },
        { id: 'u-003', username: 'cristiano', fullName: 'Cristiano Ronaldo', avatar: 'https://i.pravatar.cc/150?u=cr7' }
      ];
      setFavoriteUsers(mockData.filter(u => response.includes(u.id)));
    } catch (error) {
  ...
  const handleSearch = async (val) => {
    ...
    if (val.trim().length > 0) {
      setIsSearching(true);
      try {
        const response = await userService.searchUsers(val);
  ...
  const handleToggleFavorite = async (user) => {
    try {
      await userService.toggleFavoriteUser(user.id);
      ...
      setAlertConfig({ isOpen: true, title: 'Error', message: 'Failed to update favorites.' });
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setAlertConfig({ isOpen: true, title: 'Success', message: 'Your profile has been saved.' });
    } catch (error) {
      setAlertConfig({ isOpen: true, title: 'Error', message: error.message || 'Failed to update profile.' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <>
      <Flex 
        maxW="1200px" width="95%" mx="auto" mt={12} 
        border="1px solid" borderColor="gray.200" minH="85vh" 
        bg="white" borderRadius="8px" overflow="hidden"
      >
        {/* Sidebar */}
        ...
        <Box flex={1} p={12} bg="white" color="black" overflowY="auto">
          {activeTab === 'edit-profile' && (
            <VStack align="stretch" gap={10} width="100%">
              ...
              <HStack gap={6} bg="gray.50" p={6} borderRadius="12px">
                <UserAvatar src={currentUser.avatar} size="60px" />
                <VStack align="start" gap={0}>
                  <Text fontWeight="bold" fontSize="18px">{currentUser.username}</Text>
                  <Text color="#0095f6" fontSize="14px" fontWeight="bold" cursor="pointer" _hover={{ color: "blue.800" }}>Change profile photo</Text>
                </VStack>
              </HStack>
              ...
              <VStack align="stretch" gap={6}>
                <Box>
                  ...
                  <Input 
                    ...
                    bg="white" border="1px solid" borderColor="gray.300" fontSize="16px" py={6}
                ...
                <Box>
                  ...
                  <Textarea 
                    ...
                    bg="white" border="1px solid" borderColor="gray.300" fontSize="16px" minH="120px"
                    _focus={{ borderColor: "gray.400", boxShadow: "none" }}
                  />
                ...
                <Box>
                  ...
                  <NativeSelect.Root>
                    <NativeSelect.Field 
                      ...
                      bg="white" color="black" border="1px solid" borderColor="gray.300"
                      fontSize="16px" height="50px" _focus={{ borderColor: "gray.400", boxShadow: "none" }}
                    >
              ...
              <Button 
                bg="#0095f6" _hover={{ bg: "#1877f2" }} color="white" 
                w="fit-content" px={10} py={6} fontSize="16px" fontWeight="bold"
                borderRadius="8px" alignSelf="flex-end" onClick={handleSubmit} isLoading={isLoading}
              >
                Submit
              </Button>
            </VStack>
          ...
          {activeTab === 'favorites' && (
            <VStack align="stretch" gap={6} width="100%">
              ...
              <Text fontSize="16px" color="gray.500">Manage who you want to see first in your feed. Only you can see your favorites list.</Text>
              
              <VStack align="stretch" gap={3} mt={4}>
                <HStack bg="gray.100" px={4} py={1} borderRadius="10px">
                  <BsSearch color="gray" />
                  <Input 
                    placeholder="Search" variant="plain" fontSize="14px" 
                    value={searchQuery} onChange={(e) => handleSearch(e.target.value)}
                    border="none" _focus={{ boxShadow: "none" }} p={2} color="black"
                  />
                ...
                {favoriteUsers.length > 0 ? (
                  favoriteUsers.map((user) => (
                    <Flex key={user.id} justify="space-between" align="center" p={4} bg="gray.50" borderRadius="12px">
                      ...
                      <IconButton 
                        icon={<BsStarFill />} variant="ghost" color="black" 
                        aria-label="Remove favorite" onClick={() => handleToggleFavorite(user)}
                      />
                    </Flex>
                  ))
                ) : (
            ...
            <VStack align="stretch" gap={8} width="100%">
              ...
              <Flex justify="space-between" align="center" p={6} border="1px solid" borderColor="gray.100" borderRadius="12px">
                <VStack align="start" gap={2} maxW="80%">
                  ...
                  <Text fontSize="14px" color="gray.500">When your account is private, only people you approve can see your photos and videos on Instagram.</Text>
                </VStack>
                <Box
                  as="button" p={1} width="50px" height="28px" borderRadius="full"
                  bg={isPrivate ? "#0095f6" : "gray.300"} transition="0.3s"
                  onClick={() => setIsPrivate(!isPrivate)} position="relative"
                >
          ...
          {activeTab === 'blocking' && (
            <VStack align="stretch" gap={6} width="100%">
              ...
              <Text fontSize="16px" color="gray.500">Once you block someone, that person can no longer see your profile or posts on Instagram.</Text>
              
              <VStack align="stretch" gap={4} mt={6}>
                {blockedUsers.length > 0 ? (
                  blockedUsers.map(user => (
                    <Flex key={user.id} align="center" justify="space-between" p={4} bg="gray.50" borderRadius="12px">
                      <HStack gap={4}>
                        <UserAvatar src={user.avatar} size="50px" />
                        <VStack align="start" gap={0}>
                          <Text fontWeight="bold" fontSize="16px">{user.username}</Text>
                      ...
                      <Button variant="outline" borderColor="gray.300" size="md" onClick={() => unblockUser(user.id)} borderRadius="8px">
                        Unblock
                      </Button>
                    </Flex>
                  ))
                  ...
                  <Flex direction="column" align="center" justify="center" py={20}>
                    <Text color="gray.400" fontSize="16px">You haven't blocked anyone yet.</Text>
                  </Flex>
                )}
              </VStack>
            </VStack>
