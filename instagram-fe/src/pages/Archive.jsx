...
import { 
  Box, VStack, HStack, Text, Icon, Grid, Image, 
  Flex, IconButton, SimpleGrid
...
const Archive = () => {
  ...
  return (
    <Box bg="white" minH="100vh" color="black">
      ...
      <Flex align="center" px={4} py={3} borderBottom="1px solid" borderColor="gray.100">
        <IconButton
        ...
        />
        <Text fontSize="20px" fontWeight="700">Archive</Text>
      </Flex>

      {/* 2. Tabs Section */}
      <Flex justify="center" borderBottom="1px solid" borderColor="gray.100">
        <HStack gap={12}>
          <VStack 
            gap={2} py={3} cursor="pointer" position="relative" 
            onClick={() => setActiveTab('stories')}
            color={activeTab === 'stories' ? 'black' : 'gray.400'}
          >
            <HStack>
              <Icon as={BsClockHistory} boxSize={5} />
              <Text fontWeight="700" fontSize="12px" letterSpacing="1px">STORIES</Text>
            </HStack>
            {activeTab === 'stories' && <Box h="1px" w="100%" bg="black" position="absolute" bottom="0" />}
          </VStack>
          ...
          >
            <HStack>
              ...
              <Text fontWeight="700" fontSize="12px" letterSpacing="1px">POSTS</Text>
            </HStack>
            {activeTab === 'posts' && <Box h="1px" w="100%" bg="black" position="absolute" bottom="0" />}
          </VStack>
        </HStack>
      </Flex>
          ...
          <Grid templateColumns="repeat(4, 1fr)" gap={1} px={1}>
            {archivedStories.map((story, index) => (
              ...
              >
                ...
                />
                <Box 
                  position="absolute" top="8px" left="8px" bg="white" borderRadius="6px" 
                  px={1.5} py={0.5} boxShadow="0 2px 4px rgba(0,0,0,0.15)" textAlign="center" minW="34px"
                >
                  <Text color="black" fontSize="15px" fontWeight="800" lineHeight="1" mb="2px">{story.day}</Text>
                  <Text color="black" fontSize="9px" fontWeight="700" lineHeight="1">{story.month}</Text>
                </Box>
              </Box>
            ))}
          </Grid>
