...
const ReelCard = ({ reel, onOpenComments, onOpenMoreOptions }) => {
    ...
    const [isLiked, setIsLiked] = useState(
        reel.likedBy?.some((u) => u.id === authUser?.id) || false,
    );
    const [isPlaying, setIsPlaying] = useState(true);
    const [showHeartAnim, setShowHeartAnim] = useState(false);

    const isOwnPost =
        authUser?.id === reel.userId || authUser?.id === reel.user?.id;
    ...
    return (
        ...
        >
            ...
            >
                ...
                <Text fontSize="16px" mb={4} noOfLines={2}>
                    {reel.caption}
                </Text>
                <HStack gap={2}>
...
const Reels = () => {
    ...
    const isOwnActiveReel =
        authUser?.id === activeReel?.userId ||
        authUser?.id === activeReel?.user?.id;
