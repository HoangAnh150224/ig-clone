...
const postService = {
  ...
  getFeedPosts: async (userId) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const allFeedItems = [
          ...allPosts.filter(post => post.userId === userId || followingIds.includes(post.userId))
            .map(p => ({ 
              ...p, 
              type: 'post',
              user: usersDB[p.userId]
            })),
          ...allReels.filter(reel => reel.userId === userId || followingIds.includes(reel.userId))
            .map(r => ({ 
              ...r, 
              type: 'reel',
  ...
  getExplorePosts: async (userId) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const exploreReels = allReels.filter(reel =>
          reel.userId !== userId && !followingIds.includes(reel.userId)
        ).map(reel => ({ ...reel, type: 'reel' }));

        const combined = [...explorePosts, ...exploreReels].sort(() => Math.random() - 0.5);
  ...
  getUserPosts: async (userId) => {
    return new Promise((resolve) => {
      setTimeout(() => {
        ...
        const userReels = allReels.filter(reel => reel.userId === userId)
          .map(reel => ({ ...reel, type: 'reel' }));
  ...
  getFavoritePosts: async () => {
    return new Promise((resolve) => {
      setTimeout(() => {
        const favPosts = allPosts.slice(0, 3).map(p => ({ ...p, type: 'post' }));
        resolve(favPosts);
      }, 500);
    });
  },
