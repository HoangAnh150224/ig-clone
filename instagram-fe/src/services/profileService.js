import { allUsers, currentUser, contentDB, userRelationsDB, commentsDB, usersDB, allReels } from '../api/dummyData';
...
const profileService = {
  ...
  getUserProfile: async (username) => {
    return new Promise((resolve) => {
      ...
      setTimeout(() => {
        ...
        const userPostsCount = contentDB.posts.filter(p => p.userId === rawUser.id).length;
        const userReelsCount = contentDB.reels.filter(r => r.userId === rawUser.id).length;
  ...
  addToHighlight: async (storyId, highlightId = null, newTitle = null) => {
    return new Promise((resolve) => {
      console.log(`Backend Action: Adding Story ${storyId} to Highlight ${highlightId || 'New: ' + newTitle}`);
      
      // Simulate backend processing
      setTimeout(() => {
        // Find story in current user's archive (u-001)
        const story = currentUser.archivedStories?.find(s => s.id === storyId);
