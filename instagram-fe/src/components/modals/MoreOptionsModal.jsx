...
const OptionButton = ({ label, color = "black", fontWeight = "normal", onClick }) => (
  <Box
    width="100%"
    py={3.5}
    textAlign="center"
...
const MoreOptionsModal = ({ 
  isOpen, onClose, isOwnPost, post, 
  isProfile = false, isOwnProfile = false, user, 
  onParentClose, isFavoriteUser = false, isBlocked = false 
}) => {
  ...
  const handleAction = async (action, extraData = null) => {
    ...
    try {
      let response;
      const targetUserId = isProfile ? user?.id : (post?.userId || post?.user?.id);
      ...
      switch (action) {
        ...
        case 'report_init':
          ...
          return;
        case 'report_submit':
          response = await postService.reportPost(targetUserId, extraData);
          if (response?.success) {
        ...
        case 'toggle_favorite_user':
          response = await userService.toggleFavoriteUser(targetUserId);
          break;
        case 'copy_link':
          const linkToCopy = isProfile 
            ? window.location.origin + `/${user.username}` 
            : (post.shareUrl || window.location.origin + `/p/${post.id}`);
  ...
  return (
    <>
      ...
      >
        ...
        <DialogPositioner>
          <DialogContent borderRadius="12px" overflow="hidden" bg="white" p={0} maxW="400px" width="90vw">
            <DialogBody p={0}>
              <VStack gap={0}>
                {view === 'main' ? (
                  /* MAIN MENU */
                  <>
                    {isProfile ? (
                      isOwnProfile ? (
                        <>
                          ...
                          <OptionButton label="Archive" onClick={() => handleAction('archive')} />
                          <Box width="100%" height="1px" bg="gray.100" />
                          <OptionButton label="QR Code" onClick={() => handleAction('qr')} />
                        </>
                      ) : (
                        <>
                          {isBlocked ? (
                            <OptionButton label="Unblock" color="#0095f6" fontWeight="bold" onClick={() => handleAction('unblock')} />
                          ) : (
                            <OptionButton label="Block" color="#ed4956" fontWeight="bold" onClick={() => handleAction('block_init')} />
                          )}
                          <Box width="100%" height="1px" bg="gray.100" />
                          <OptionButton label="Restrict" color="#ed4956" fontWeight="bold" onClick={() => handleAction('restrict')} />
                          <Box width="100%" height="1px" bg="gray.100" />
                    ...
                    {!isProfile && (
                      <>
                        <Box width="100%" height="1px" bg="gray.100" />
                        <OptionButton 
                          label={isFavoriteUser ? "Remove from favorites" : "Add to favorites"} 
                          color={isFavoriteUser ? "#ed4956" : "black"}
                    ...
                    <Box width="100%" height="1px" bg="gray.100" />
                    <OptionButton label="Share to..." onClick={() => handleAction('share')} />
                    <Box width="100%" height="1px" bg="gray.100" />
                  ...
                  <Box width="100%">
                    <Box py={6} px={10} textAlign="center">
                      <Text fontWeight="600" fontSize="18px" mb={2}>Block {isProfile ? user?.username : post?.user?.username}?</Text>
                      <Text fontSize="14px" color="gray.500">
                        They won't be able to find your profile, posts or story on Instagram. They won't know you blocked them.
                      </Text>
                    </Box>
                    {blockSuccess ? (
                      <Box p={8} textAlign="center" borderTop="1px solid" borderColor="gray.100">
                        <Text fontSize="14px" color="#ed4956" fontWeight="600">
                          User blocked.
                        </Text>
                      </Box>
                    ) : (
