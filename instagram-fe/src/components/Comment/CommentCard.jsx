  ...
  return (
    <Box mb={4} width="100%">
      <Flex gap={3} align="start" width="100%">
        <Box cursor="pointer" onClick={() => handleNavigate(comment.user.username)}>
          <UserAvatar src={comment.user.avatar} size="32px" />
        </Box>
        <Box flex={1}>
          <Text fontSize="14px" lineHeight="1.4" color="black">
            <Text as="span" fontWeight="bold" mr={2} cursor="pointer" onClick={() => handleNavigate(comment.user.username)} _hover={{ opacity: 0.7 }}>
              {comment.user.username}
            </Text>
            {comment.content}
          </Text>
          <HStack gap={4} mt={2}>
            <Text fontSize="12px" color="gray.500">{comment.timeAgo}</Text>
            {likeCount > 0 && (
              <Text 
                fontSize="12px" color="gray.500" fontWeight="bold" cursor="pointer" 
                _hover={{ color: "black" }} onClick={() => setIsListOpen(true)}
              >
      ...
      {comment.replies && comment.replies.length > 0 && (
        <Box ml="44px" mt={3}>
          {!showReplies ? (
            <Text fontSize="12px" color="gray.500" fontWeight="bold" cursor="pointer" onClick={() => setShowReplies(true)}>
              ——— View all {comment.replies.length} replies
            </Text>
          ) : (
            <VStack align="stretch" gap={4} mt={2}>
              <Text fontSize="12px" color="gray.500" fontWeight="bold" cursor="pointer" onClick={() => setShowReplies(false)}>
                ——— Hide replies
              </Text>
...
const ReplyItem = ({ reply, onNavigate }) => {
  ...
  return (
    <Flex gap={3} align="start">
      <Box cursor="pointer" onClick={() => onNavigate(reply.user.username)}>
        <UserAvatar src={reply.user.avatar} size="24px" />
      </Box>
      <Box flex={1}>
        <Text fontSize="14px" color="black" lineHeight="1.4">
          <Text as="span" fontWeight="bold" mr={2} cursor="pointer" onClick={() => onNavigate(reply.user.username)}>
            {reply.user.username}
          </Text>
          {reply.content}
        </Text>
        <HStack gap={4} mt={1}>
          <Text fontSize="12px" color="gray.500">{reply.timeAgo}</Text>
          {likeCount > 0 && (
            <Text 
              fontSize="12px" color="gray.500" fontWeight="bold" cursor="pointer" 
      ...
      <UserListModal isOpen={isListOpen} onClose={() => setIsListOpen(false)} title="Likes" users={likedUsers} />
    </Flex>
  );
};
